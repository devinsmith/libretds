 ...
 * \remark The RPC functions are the only way to get back OUTPUT parameter
 *         data with db-lib from modern Microsoft servers.
...
dbrpcinit(DBPROCESS * dbproc, const char rpcname[], DBSMALLINT options)
{
   ...
   *  If rpcname is specified, that new stored procedure is initialized after
  ...
  options &= ~DBRPCRECOMPILE; /* turn recompile off, now that it's extracted */
  ...
  for (rpc = &dbproc->rpc; *rpc != NULL; rpc = &(*rpc)->next) {
    ...
    if ((*rpc)->name == NULL  || strcmp((*rpc)->name, rpcname) == 0) {
      tdsdump_log(TDS_DBG_INFO1,
        "error: dbrpcinit called twice for procedure \"%s\"\n", rpcname);
      return FAIL;
    }
  }

  /* rpc now contains the address of the dbproc's first empty (null)
   * DBREMOTE_PROC* */

  /* allocate */
  if ((*rpc = (DBREMOTE_PROC *) calloc(1, sizeof(DBREMOTE_PROC))) == NULL) {
    dbperror(dbproc, SYBEMEM, errno, __func__, __LINE__);
    return FAIL;
  }
 ...
 * \param dbproc contains all information needed by db-lib to manage
 *               communications with the server.
 * \param paramname literal name of the parameter, according to the stored
 *                  procedure (starts with '@').  Optional.
  *                 If not used, parameters will be passed in order instead of
  *                 by name.
 * \param status must be DBRPCRETURN, if this parameter is a return parameter,
 *               else 0.
 * \param type datatype of the value parameter e.g., SYBINT4, SYBCHAR.
 * \param maxlen Maximum output size of the parameter's value to be returned
 ...
 */
RETCODE
dbrpcparam(DBPROCESS * dbproc, const char paramname[], BYTE status,
    int db_type, DBINT maxlen, DBINT datalen, BYTE * value)
{
  char *name = NULL;
  DBREMOTE_PROC *rpc;
  ...
  if (is_fixed_type(type)) {
    ...
    /* Sybooks: "Passing datalen as -1 for any of these [non-fixed] datatypes
     * results in the DBPROCESS referenced by dbproc being marked as "dead,"
     * or unusable." */
    DBPERROR_RETURN(datalen < 0, SYBERPIL);
  }

  /* "value parameter for dbprcparam() can be NULL, only if the datalen
   * parameter is 0." */
  ...
  if (status & DBRPCRETURN) {
     ...
     */
    DBPERROR_RETURN3(maxlen != -1 && maxlen != 0, SYBEIPV, (int) maxlen,
        "maxlen", "dbrpcparam");
    maxlen = -1;
  }
  ...
  if (param == NULL) {
    dbperror(dbproc, SYBEMEM, 0, __func__, __LINE__);
    return FAIL;
  }
  ...
  /* To specify that an input parameter is NULL it is necessary to explicitly
   * set the value to NULL. Previously it was ok to just set datalen to 0,
   * however this causes some problems with empty strings.
   * i.e. strlen("") == 0 */
  ...
  /* find "current" procedure */
  for (rpc = dbproc->rpc; rpc->next != NULL; rpc = rpc->next);
...
dbrpcsend(DBPROCESS * dbproc)
{
  ...
  CHECK_CONN(FAIL);
  /* dbrpcinit should allocate pointer */
  CHECK_PARAMETER(dbproc->rpc, SYBERPCS, FAIL);

  /* sanity */
  if (dbproc->rpc->name == NULL) {
    /* can't be ready without a name */
...
/** 
 * Tell the TDSPARAMINFO structure where the data go.  This is a kind of
 * "bind" operation.
 */
static const unsigned char *
param_row_alloc(TDSPARAMINFO *params, TDSCOLUMN *curcol, int param_num,
    void *value, int size)
{
  ...
  if (size > 0 && value) {
    ...
    if (value != NULL) {
      tdsdump_log(TDS_DBG_FUNC, "setting parameter #%d to 0 bytes\n",
          param_num);
