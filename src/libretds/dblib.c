...
dblogin(void)
{
	...
	if ((loginrec = (LOGINREC*) malloc(sizeof(LOGINREC))) == NULL) {
		dbperror(NULL, SYBEMEM, errno, __func__, __LINE__);
		return NULL;
	}
 ...
 */
DBPROCESS *
tdsdbopen(LOGINREC * login, const char *server)
{
	...
	if ((dbproc->tds_socket = tds_alloc_socket(dblib_get_tds_ctx(), 512)) == NULL ){
		dbperror(NULL, SYBEMEM, 0, __func__, __LINE__);
		free(dbproc);
		return NULL;
	}
...
default_err_handler(DBPROCESS * dbproc, int severity, int dberr, int oserr, char *dberrstr, char *oserrstr)
{
	...
	if (dbproc == NULL) {	/* i.e. Sybase behavior */
		switch(dberr) {
		case SYBETIME:
			return INT_EXIT;
		default: 
...
static const DBLIB_ERROR_MESSAGE dblib_error_messages[] = 
	...
	, { SYBEMEM,           EXRESOURCE,	"Unable to allocate sufficient memory (%1!:%2!)\0%s%d" }
